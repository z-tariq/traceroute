...
func (options *TracerouteOptions) SetMaxProbes(maxProbes int) {
	options.probes = maxProbes
}

func (options *TracerouteOptions) MaxProbes() int {
	if options.probes == 0 {
		options.probes = DEFAULT_PROBES
	}
	return options.probes
}
...
func Traceroute(dest string, options *TracerouteOptions, c ...chan TracerouteHop) (result TracerouteResult, err error) {
	...
	retry := 0
	for {
		for probe := 0; probe < options.MaxProbes(); probe++ {
